...
* Essentially what fuff does is that it takes a list of different words and tries them againts a specific part of the URL
...
What I did here is that I downloaded an module to unpack the zip file found in teros website.
...
So after iterating trough different LIMIT versions I ended up with the correct flag.
...
# d) Break into 020-your-eyes-only. See Karvinen 2024: Hack'n Fix --UPDATE COMING FOR THE REST OF THE EXERCISES, I RAN OUT OF TIME :/--

------UPDATED 25.01.2026 12:00 - 13:30--------
...
Here i just ran a ffuf command that I previously went in depth to. I found a directory called admin-console

<img width="979" height="522" alt="image" src="https://github.com/user-attachments/assets/570a302d-c953-46a9-8edf-e5cfc1a7e0ed" />

So this exercise as a bit tricky, I found the admin-console but that took me to nowhere.

So i went to the tips because i did not understand what I needed to do here. From the tips it said to try as a logged in user.

But this was not as straightforward either, since after registering an accoung and logging in I tought i was logged in (I just tought that the login screen was buggy or something and it did not show me that i am actually logged in)

So I tried to access the admin console again and again and tried to manipulate the source even but nothing produced results.

So I went and read the walktrough by robin : https://rbin.dev/diary/entries/diary.html?entry=SH24-002&week=

I just had to create the login with the instructions on the register webpage (Im pretty dumb)

After that I got access to the test page login screen 

<img width="1891" height="625" alt="image" src="https://github.com/user-attachments/assets/2196f195-42fc-4456-b3e7-fbbd88b6a7d8" />


# e) Fix the 020-your-eyes-only vulnerability. Demonstrate with a test that your solution works.

Analyzing a codebase with nano was pretty tedious so I opted to use windows vscode for this exercise.

I can guess that I need to code some sort of a check that determins if i am an admin or an user and based on that show me X things. 

Now after a little chatting with an AI (I did not ask straight up tell me the answer to this but rather explain what I am seeing here) I could determine that the problem is in the AdminDashboardView,

<img width="836" height="522" alt="image" src="https://github.com/user-attachments/assets/fcee2a59-7c99-421a-8bba-b94dc34888fa" />

There is a check implemented here allready but I guess it has been left out, I think that if I just copy the self.request.user.is_staff then i should be able to get it to work.

<img width="1305" height="693" alt="image" src="https://github.com/user-attachments/assets/e36da101-259e-4dad-972c-78f9f1218d9c" />

After implemeting the check, if i try to access the admin console as normal user priviledges I get the 403 error

<img width="1708" height="637" alt="image" src="https://github.com/user-attachments/assets/65928be8-8bcd-405f-9ec3-e479ab1da7f9" />
